<x-filament-panels::page>
    <div class="space-y-6" wire:poll.5s="refreshBalances">
            ...
            <div class="bg-white dark:bg-gray-900 border border-emerald-200/80 dark:border-emerald-800/60 rounded-2xl p-5 shadow-xs hover:border-emerald-300 dark:hover:border-emerald-700 transition-all flex flex-col justify-between h-full">
                ...
                <div class="mt-4 pt-1">
                    ...
                    <div class="mt-1.5 flex items-center justify-between gap-2 text-xs">
                        <span class="text-emerald-700/80 dark:text-emerald-400/80 truncate">Porsi Dev</span>
                        @if($devUnpaidBalance > 0)
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold bg-emerald-100 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-200 border border-emerald-200/80 dark:border-emerald-800/80 whitespace-nowrap shrink-0">
                                Sisa: Rp {{ number_format($devUnpaidBalance, 0, ',', '.') }}
                            </span>
                        @else
                            <span class="text-xs font-semibold text-emerald-600/80 dark:text-emerald-400/80 whitespace-nowrap">Lunas Terbayar</span>
                        @endif
                    </div>
                </div>
            </div>
        ...
        @if (auth()->user()?->hasRole('ryu_dev') && !auth()->user()?->hasRole('super_admin'))
            {{-- TRANSACTIONS TABLE FOR RYU_DEV ROLE (WITHOUT TABS) --}}
            <div class="space-y-4">
                {{ $this->table }}
            </div>
        @else
