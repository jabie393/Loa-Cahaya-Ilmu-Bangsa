<x-filament-panels::page>
    <!-- Filter Website Jurnal -->
    <div
        class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
        <div>
    ...
    <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        @foreach ($this->getJurnals() as $jurnal)
            <div
                class="group relative flex flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_40px_80px_rgba(37,99,235,0.08)] dark:border-slate-800 dark:bg-slate-900">
                <!-- Cover Image Container -->
                <div class="relative aspect-[3/4] overflow-hidden">
                    <img src="{{ $jurnal['image'] }}" alt="{{ $jurnal['name'] }}"
                        class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110">

                    <!-- Overlay Gradient -->
                    <div
                        class="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100">
                    </div>
                </div>
                ...
                <div class="flex flex-1 flex-col p-6">
                    <h3
                        class="mb-2 text-xl font-bold tracking-tight text-slate-900 transition-colors duration-300 group-hover:text-blue-600 dark:text-white">
                        {{ $jurnal['name'] }}
                    </h3>
                    <!-- Buttons Group -->
                    <div class="mt-auto flex flex-col gap-2">
                        @if(!empty($jurnal['template_link']))
                            <a href="{{ $jurnal['template_link'] }}" target="_blank"
