<div class="space-y-6">
    <div
        ...
        <p class="text-primary-900 dark:text-primary-100 text-sm font-medium">Silakan periksa kembali data Anda sebelum
            melakukan pengiriman.</p>
    </div>

    <div class="grid grid-cols-1 gap-6 pb-6 md:grid-cols-2">
        <!-- Review Data Left Column -->
        <div class="space-y-4">
            <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Informasi Penulis &
                Publikasi</h4>
            <div
                ...
                <div class="flex flex-col">
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Email</span>
                    <span
                        class="text-sm font-semibold text-gray-900 break-words dark:text-white">{{ $get('email') }}</span>
                </div>
                <div class="flex flex-col">
        ...
        <div class="space-y-4">
            ...
            <div
                class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:border-gray-700 dark:bg-gray-800">
                <div class="flex items-center gap-3">
                    <div
                        class="rounded-lg bg-red-50 dark:bg-red-900/20 p-3 text-red-600 dark:text-red-400 animate-pulse">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-gray-900 truncate dark:text-white">
                            @php
                                $manuscript = $get('manuscript_file');
                                if (is_array($manuscript)) {
                                    $manuscript = reset($manuscript);
                                }
                                $manuscriptName = null;
                                if ($manuscript) {
                                    if (is_string($manuscript)) {
                                        $manuscriptName = basename($manuscript);
                                    } elseif (method_exists($manuscript, 'getClientOriginalName')) {
                                        $manuscriptName = $manuscript->getClientOriginalName();
                                    }
                                }
                            @endphp
                            {{ $manuscriptName ?: 'File PDF Naskah' }}
                        </p>
            ...
            <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Bukti Pembayaran
            </h4>
            <div
                class="flex min-h-[200px] flex-col items-center justify-center rounded-2xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:border-gray-700 dark:bg-gray-800">
                @php
                ...
                @if ($proofUrl)
                    <div
                        class="group relative overflow-hidden rounded-xl border border-gray-100 shadow-sm transition-all hover:shadow-md dark:border-gray-700">
                        ...
                        <div
                            class="absolute inset-0 flex items-center justify-center bg-gray-900/60 opacity-0 backdrop-blur-[2px] transition duration-300 group-hover:opacity-100">
                            <button type="button" onclick="proof_modal.showModal()"
                    ...
                    <dialog id="proof_modal" class="modal modal-bottom sm:modal-middle">
                        <div
                            class="modal-box max-w-5xl p-2 bg-white/10 backdrop-blur-md shadow-none border-none overflow-hidden relative">
                            <button type="button" onclick="proof_modal.close()"
                        ...
                        <div class="modal-backdrop backdrop-blur-sm bg-black/40">
                            <button type="button" onclick="proof_modal.close()"
                                class="w-full h-full cursor-default">close</button>
                        </div>
                    </dialog>
                ...
                @else
                    <div class="py-12 text-center">
                        <div
                            class="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gray-50/50 dark:bg-gray-900/30">
                            <svg class="h-10 w-10 text-gray-200 dark:text-gray-700" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                        ...
                        <p class="mt-1 text-[10px] text-gray-400/80">Silakan unggah bukti pembayaran di langkah sebelumnya
                        </p>
                    </div>
                @endif
            </div>
