<x-filament-panels::page>
    ...
    <div x-data="bulkPaymentApp({
            ...
            simulateUrl: '{{ route('payments.simulate.bulk', ['paymentId' => $paymentId]) }}',
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        Selesaikan pembayaran untuk seluruh naskah terpilih.
                    </p>
                </div>
            </div>
            <div>
        ...
        <div class="payment-grid-layout">
            ...
            <div class="space-y-4">
                ...
                @foreach($itemsPricing as $index => $item)
                    ...
                    @endphp
                    <div
                        class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm space-y-3 relative overflow-hidden transition-all hover:border-blue-300 dark:hover:border-blue-700">
                        <div
                            class="absolute top-0 right-0 bg-slate-100 dark:bg-gray-800 px-3 py-1 rounded-bl-xl text-[10.5px] font-mono font-bold text-slate-600 dark:text-slate-400">
                            {{ $sub->id }}
                        </div>

                        <div>
                            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Naskah
                                {{ $index + 1 }}</span>
                        ...
                        <div class="grid grid-cols-2 gap-4 pt-2 border-t border-gray-100 dark:border-gray-800 text-xs">
                            <div>
                                ...
                                <span
                                    class="font-bold text-gray-800 dark:text-gray-200 block">{{ $sub->journal?->name ?? 'Jurnal CIB' }}</span>
                                <span
                                    class="inline-block mt-0.5 px-2 py-0.5 rounded text-[10px] font-semibold bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-slate-400">
                            ...
                            <div class="text-right">
                                @if(!empty($pr['is_member']) || (!empty($pr['discount_amount']) && $pr['discount_amount'] > 0))
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        <span
                                            class="text-[9.5px] font-bold px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300 border border-blue-200 dark:border-blue-800/50 whitespace-nowrap">-Rp
                                            {{ number_format($pr['discount_amount'] ?? 10000, 0, ',', '.') }}</span>
                                    </div>
                                @endif
                ...
                <div
                    ...
                    <template x-if="status === 'paid'">
                        <div class="text-center py-6 space-y-4">
                            <div
                                class="w-16 h-16 bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center mx-auto mb-2">
                                <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                                </svg>
                            </div>
                            <h3 class="text-lg font-black text-gray-900 dark:text-white">Pembayaran Kolektif Berhasil!
                            </h3>
                            <p class="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
                                Seluruh {{ count($submissions) }} naskah terpilih telah disetujui (Approved) secara
                                otomatis dan disinkronkan ke OJS.
                            </p>
                            <div class="pt-2 space-y-2">
                                <a :href="'/invoice/bulk/' + paymentId"
                                    href="{{ $paymentId ? route('public.invoice.bulk.preview', ['payment' => $paymentId]) : '#' }}"
                                    target="_blank"
                                    class="w-full inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs transition-all shadow-md cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                    </svg>
                                    <span>Download Invoice Kolektif / Bukti Bayar PDF</span>
                                </a>
                                <a href="{{ \App\Filament\Resources\Submissions\SubmissionResource::getUrl('index') }}"
                                    class="w-full inline-flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-gray-800 text-slate-700 dark:text-slate-300 font-bold py-2.5 px-4 rounded-xl text-xs transition-all">
                    ...
                    <template x-if="status === 'pending' && !isExpired">
                        <div>
                            <div
                                ...
                                <span class="text-xs font-bold text-gray-700 dark:text-gray-300">Scan QRIS
                                    Kolektif</span>
                                <div
                                    class="flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400 font-mono font-bold bg-amber-50 dark:bg-amber-950/40 px-2 py-0.5 rounded-md">
                            ...
                            <div class="mb-3 text-center">
                                <span
                                    class="text-[10px] text-gray-400 block uppercase tracking-wider font-semibold">Order
                                    ID Transaksi</span>
                            ...
                            <div
                                ...
                                <template x-if="qrisUrl && !errorMessage">
                                    <div class="flex flex-col items-center gap-3">
                                        <div class="bg-white p-3 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                                ...
                                <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-3 text-center">
                                    Buka GoPay, BCA, Mandiri, ShopeePay, Dana, atau Mobile Banking apa saja untuk scan
                                    QR di atas.
                                </p>
                            ...
                            <div
                                class="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-gray-950/60 border border-slate-200 dark:border-gray-800 space-y-2 text-xs">
                                <span
                                    class="text-[10.5px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Rincian
                                    Total Kolektif</span>
                                ...
                                <div
                                    ...
                                    <span
                                        class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-wider">Total
                                        Tagihan:</span>
                            ...
                            <div class="mt-4 space-y-2">
                                <button type="button" @click="checkStatus(false)" :disabled="isChecking"
                                    class="w-full inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold py-2.5 px-4 rounded-xl text-xs sm:text-sm transition-all shadow-md active:scale-[0.99]">
                    ...
                    <template x-if="status === 'expired' || isExpired">
                        <div class="text-center py-6 space-y-4">
                            <div
                                class="w-16 h-16 bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 rounded-full flex items-center justify-center mx-auto mb-2">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
                                Batas waktu pembayaran telah habis. Klik tombol di bawah untuk membuat QRIS kolektif
                                baru.
                            </p>
                            <div class="pt-2 space-y-2">
                                <button type="button" @click="regenerateQris()" :disabled="isRegenerating"
        ...
        function bulkPaymentApp(config) {
            return {
                ...
                async simulateSandbox() {
                    ...
                    if (!confirm('Simulasikan pembayaran kolektif sukses via Sandbox?')) return;
                ...
                async regenerateQris() {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            this.qrisUrl = data.qris_url;
                            if (data.qr_string) this.qrString = data.qr_string;
                            if (data.payment_id) this.paymentId = data.payment_id;
