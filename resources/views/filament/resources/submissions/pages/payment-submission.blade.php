<x-filament-panels::page>
    ...
    <div x-data="paymentApp({
            ...
            initialGateway: '{{ $payment ? ($payment->gateway ?: 'midtrans') : app(\App\Services\PaymentGateways\PaymentGatewayManager::class)->getActiveGatewayName() }}',
            switchGatewayUrl: '{{ route('submissions.payment.switch-gateway', $record->id) }}',
        ...
        <div
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">Selesaikan pembayaran untuk submission dan
                        Letter
                        of Acceptance (LOA).</p>
                </div>
            </div>
                                ...
                                <div
                                    ...
                                    <template x-if="errorMessage">
                                        <div
                                            ...
                                            <div class="pt-1">
                                                <template x-if="errorMessage && errorMessage.includes('maksimal 30')">
                                                    <a href="{{ \App\Filament\Resources\Submissions\SubmissionResource::getUrl('view', ['record' => $record]) }}"
                                                        class="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-lg text-xs shadow-sm transition-colors cursor-pointer">
                                    ...
                                    @if($canSwitchGateway)
                                        <div class="mt-3 w-full p-2.5 bg-gray-50 dark:bg-gray-800/80 rounded-xl border border-gray-200 dark:border-gray-700/80 text-left shadow-xs">
                                            <div class="flex items-center justify-between gap-2">
                                                <div class="flex items-center gap-1.5">
                                                    <svg class="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
                                                    </svg>
                                                    <span class="text-[11px] font-bold text-gray-700 dark:text-gray-300">Gateway:</span>
                                                    <span class="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded"
                                                          :class="gateway === 'belibayar' ? 'bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300'"
                                                ...
                                                <div class="flex items-center gap-1">
                                                    <button type="button"
                                                        ...
                                                        :class="gateway === 'midtrans' ? 'bg-amber-500 text-white shadow-xs cursor-default' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600'">
                                                        Midtrans
                                                    </button>
                                                    <button type="button"
                                                        ...
                                                        class="px-2 py-0.5 rounded text-[10px] font-bold transition-all"
                                                        :class="gateway === 'belibayar' ? 'bg-blue-600 text-white shadow-xs cursor-default' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600'">
                                                        Belibayar
                                                    </button>
                                                </div>
                                            </div>
                                            <div x-show="isSwitchingGateway" class="mt-1.5 text-[10px] text-blue-600 dark:text-blue-400 font-medium flex items-center gap-1">
                                    ...
                                    @if(false && $isSandbox)
                                        <div
                                            ...
                                            <div class="flex flex-wrap items-center gap-1.5 pt-0.5">
                                                ...
                                                @endif
                                                <button type="button" @click="simulateSandbox()"
    ...
    <script>
        function paymentApp(config) {
            return {
                ...
                gateway: config.initialGateway || 'midtrans',
                switchGatewayUrl: config.switchGatewayUrl,
                ...
                async switchGateway(targetGateway) {
                    if (this.gateway === targetGateway || this.isSwitchingGateway) return;
                    this.isSwitchingGateway = true;
                    try {
                        const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
                        const res = await fetch(this.switchGatewayUrl, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                        ...
                        if (data.success) {
                            ...
                            alert(data.message || 'Gagal mengubah gateway.');
                        }
                    } catch (e) {
                        alert('Terjadi kesalahan saat menghubungi server untuk mengubah gateway.');
                    } finally {
                        this.isSwitchingGateway = false;
                    }
                },
