...
<x-filament::dropdown :placement="($position === UserMenuPosition::Topbar) ? 'bottom-end' : 'top-end'"
    :teleport="$position === UserMenuPosition::Topbar" :attributes="
    ...
    <x-slot name="trigger">
        ...
        @else
            <button aria-label="{{ __('filament-panels::layout.actions.open_user_menu.label') }}" type="button"
                ...
                <span @if ($isSidebarCollapsibleOnDesktop) x-show="$store.sidebar.isOpen" @endif
                    class="fi-user-menu-trigger-text inline-flex items-center gap-1.5 min-w-0">
                    <span class="truncate">{{ filament()->getUserName($user) }}</span>

                    @if($user && method_exists($user, 'isMember') && $user->isMember())
                        <span class="inline-flex items-center text-blue-500 hover:text-blue-600 transition-colors shrink-0"
                            x-tooltip.raw="Membership CIB" title="Membership CIB">
    ...
    @if ($hasProfileHeader)
        ...
        <x-filament::dropdown.header :color="$itemColor" :icon="$itemIcon">
            <span class="inline-flex items-center gap-1.5">
                ...
                @if($user && method_exists($user, 'isMember') && $user->isMember())
                    <span class="inline-flex items-center text-blue-500 shrink-0" x-tooltip.raw="Membership CIB"
                        title="Membership CIB">
                        <svg class="w-3.5 h-3.5 text-blue-500 fill-current" viewBox="0 0 24 24">
                            <path fill-rule="evenodd"
                                d="M8.603 3.799A4.49 4.49 0 0 1 12 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 0 1 3.498 1.307 4.491 4.491 0 0 1 1.307 3.497A4.49 4.49 0 0 1 21.75 12a4.49 4.49 0 0 1-1.549 3.397 4.491 4.491 0 0 1-1.307 3.497 4.491 4.491 0 0 1-3.497 1.307A4.49 4.49 0 0 1 12 21.75a4.49 4.49 0 0 1-3.397-1.549 4.49 4.49 0 0 1-3.498-1.306 4.491 4.491 0 0 1-1.307-3.498A4.49 4.49 0 0 1 2.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 0 1 1.307-3.497 4.49 4.49 0 0 1 3.497-1.307Zm7.007 6.387a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z"
                                clip-rule="evenodd" />
                        </svg>
